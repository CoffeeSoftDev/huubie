...
class App extends Templates {
    ...
    async showOrder(id) {
        // Use the enhanced modal from coffee.js
    ...
    async showOrderDetails(orderId) {
        ...
        if (!$('#order-details-enhanced-styles').length) {
            $("<style id='order-details-enhanced-styles'>").text(`
                :root {
    ...
    createProductDisplayArea(products) {
        if (!products || products.length === 0) {
            return `
                <div class="bg-[#283341] rounded-lg p-8 text-center h-full flex flex-col items-center justify-center">
                    <i class="icon-basket text-gray-500 text-4xl mb-4"></i>
        ...
        return `
            <div class="bg-[#2C3E50] rounded-lg p-6">
                <div class="flex items-center gap-6">
                    ...
                    <div class="w-44 h-44 rounded-xl overflow-hidden bg-[#1F2A37] flex-shrink-0">
                        ${this.renderProductImage(product)}
                    </div>
                    
                    <!-- Product Name and Price -->
                    <div class="flex-1 min-w-[200px]">
                        <h4 class="text-white font-bold text-2xl mb-3 uppercase">${product.name || 'Producto sin nombre'}</h4>
                        <p class="text-blue-400 font-semibold text-xl">$${parseFloat(product.price || 0).toFixed(2)} c/u</p>
                    </div>
                    
                    <!-- Dedication -->
                    <div class="flex-1 min-w-[180px]">
                        <span class="text-gray-400 text-sm font-medium block mb-1">Dedicatoria:</span>
                        ${hasDedication ? 
                    ...
                    <!-- Observations -->
                    <div class="flex-1 min-w-[180px]">
                        <span class="text-gray-400 text-sm font-medium block mb-1">Observaciones:</span>
                        ${hasDetails ? 
                    ...
                    <!-- Quantity (Right aligned, no background) -->
                    <div class="text-right min-w-[100px]">
                        <span class="text-gray-400 text-sm font-medium block mb-1">Cantidad:</span>
                        <p class="text-white font-bold text-2xl">${product.quantity || 1}</p>
                    </div>
                    
                    <!-- Total (Far Right, aligned with other fields) -->
                    <div class="text-right min-w-[160px]">
                        <span class="text-gray-400 text-sm font-medium block mb-1">Total:</span>
                        <p class="text-white font-bold text-3xl">$${total.toFixed(2)}</p>
                    </div>
                </div>
            </div>
        `;
    ...
    createPersonalizedCard(product) {
        ...
        return `
            <div class="bg-gradient-to-r from-purple-900/20 to-pink-900/20 rounded-lg p-6 border-2 border-purple-500/30">
                <!-- Header with Badge -->
                <div class="flex items-center justify-between mb-4">
                    <h4 class="text-white font-bold text-xl">${product.name || 'Pastel Personalizado'}</h4>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ${hasImages ? this.renderUploadedImages(product.images) : ''}
                        
                        <div class="bg-[#1F2A37] rounded-lg p-4">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-gray-400">Cantidad:</span>
                                <span class="bg-purple-600 text-white px-3 py-1 rounded font-medium">${product.quantity || 1}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-400">Precio Base:</span>
                                <span class="text-purple-300 font-semibold text-lg">${parseFloat(product.price || 0).toFixed(2)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ${hasCustomization ? this.renderCustomizationDetails(product.customer_products) : ''}
                        ${this.renderPersonalizedPricing(product)}
                    </div>
                </div>
                ...
                <div class="mt-4 pt-4 border-t border-purple-500/30">
                    ${hasDedication ? `
                        <div class="mb-3">
                            ...
                            <p class="text-gray-300 mt-1 bg-[#1F2A37] p-3 rounded">${product.dedication}</p>
                        </div>
                    ` : ''}
                    ${hasDetails ? `
                        <div>
                            <span class="text-purple-300 font-medium">Observaciones:</span>
                            <p class="text-gray-300 mt-1 bg-[#1F2A37] p-3 rounded">${product.order_details}</p>
                        </div>
                    ` : ''}
                </div>
                ` : ''}
