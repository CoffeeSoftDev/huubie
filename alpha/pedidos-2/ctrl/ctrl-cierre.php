...
class ctrl extends mdl {
    ...
    function showCorteCaja() {
        ...
        $efectivo      = floatval(!empty($payments['Efectivo']) ? $payments['Efectivo'] : 0);
        $tarjeta       = floatval(!empty($payments['Tarjeta']) ? $payments['Tarjeta'] : 0);
        $transferencia = floatval(!empty($payments['Transferencia']) ? $payments['Transferencia'] : 0);
...
function statusCorte($statusId) {
    ...
    $style = !empty($statuses[$statusId]) ? $statuses[$statusId] : ['bg' => 'bg-gray-100', 'text' => 'text-gray-700', 'label' => 'Desconocido'];
