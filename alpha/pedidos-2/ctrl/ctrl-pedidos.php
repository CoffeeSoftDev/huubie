...
class ctrl extends mdl {
    ...
    function checkDailyClosure() {
        ...
        return [
            ...
            'closure_id'     => !empty($closure['id']) ? $closure['id'] : null,
            'closed_by'      => !empty($closure['closed_by_name']) ? $closure['closed_by_name'] : null,
            'closed_at'      => !empty($closure['created_at']) ? $closure['created_at'] : null,
    ...
    public function listOrders() {
        ...
        $orders   = $this->getOrders([
            ...
            'fi'              => !empty($_POST['fi']) ? $_POST['fi'] : '',
            'ff'              => !empty($_POST['ff']) ? $_POST['ff'] : '',
            'status'          => $_POST['status'],
        ...
        foreach ($orders as $order) {
            ...
            $advanceExtra = 0;
            $discount     = !empty($order['discount']) ? $order['discount'] : 0;
            $total        = !empty($order['total_pay']) ? $order['total_pay'] : 0;
            ...
            $rows[] = [
                ...
                'dropdown'        => dropdownOrder($order['id'], $order['idStatus'], floatval(!empty($order['discount']) ? $order['discount'] : 0)),
            ];
        }
    ...
    function getOrderDetails() {
        ...
        if ($order) {
            ...
            $discount    = !empty($orderData['discount']) ? $orderData['discount'] : 0;
            $total       = !empty($orderData['total_pay']) ? $orderData['total_pay'] : 0;
            $saldo       = $total - $discount - $totalPagado;
    ...
    function initHistoryPay(){
          // Obtener sucursal para el folio
        $SUB      = !empty($_SESSION['SUB']) ? $_SESSION['SUB'] : 4;
        ...
        $PaymentsDetails     = 0;
        $discount            = !empty($ls[0]['discount']) ? $ls[0]['discount'] : 0;
    ...
    function getDashboardMetrics() {
        ...
        $subsidiariesId = !empty($_SESSION['SUB']) ? $_SESSION['SUB'] : 1;
        ...
        $data = [
            ...
            'completedSales' => [
                'count' => !empty($completedSales['count']) ? $completedSales['count'] : 0,
                'amount' => !empty($completedSales['amount']) ? $completedSales['amount'] : 0
            ],
            'pendingSales' => [
                'count' => !empty($pendingSales['count']) ? $pendingSales['count'] : 0,
                'amount' => !empty($pendingSales['amount']) ? $pendingSales['amount'] : 0
            ],
            'chartData' => $chartData,
            'previousTotalOrders' => count($prevTotalOrders),
            'previousCompletedSales' => [
                'count' => !empty($prevCompletedSales['count']) ? $prevCompletedSales['count'] : 0,
                'amount' => !empty($prevCompletedSales['amount']) ? $prevCompletedSales['amount'] : 0
            ],
            'previousPendingSales' => [
                'count' => !empty($prevPendingSales['count']) ? $prevPendingSales['count'] : 0,
                'amount' => !empty($prevPendingSales['amount']) ? $prevPendingSales['amount'] : 0
            ]
        ];
    ...
    function reportVentas() {
        $sucursal = !empty($_POST['sucursal']) ? $_POST['sucursal'] : 'all';
        $fechaInicio = !empty($_POST['fechaInicio']) ? $_POST['fechaInicio'] : date('Y-m-01');
        $fechaFin = !empty($_POST['fechaFin']) ? $_POST['fechaFin'] : date('Y-m-t');
        ...
        if ($totalPedidos > 0) {
            foreach ($orders as $order) {
                $total = floatval($order['total_pay'] - (!empty($order['discount']) ? $order['discount'] : 0));
        ...
        if (is_array($productos) && count($productos) > 0) {
            foreach ($productos as $index => $producto) {
                $data = [
                    ...
                    'total' => evaluar(floatval(!empty($producto['total']) ? $producto['total'] : 0))
                ];
                ...
                $allProductos[] = [
                    ...
                    'Total Ventas' => evaluar(floatval(!empty($producto['total']) ? $producto['total'] : 0))
                ];
            }
        } 
        ...
        if ($totalClientes > 0) {
            foreach ($clientes as $index => $cliente) {
                ...
                $data = [
                    ...
                    'name' => $cliente['name'],
                    'purchases' => $purchases,
                    'total' => evaluar(floatval(!empty($cliente['total']) ? $cliente['total'] : 0))
                ];
                ...
                $allClientes[] = [
                    ...
                    'Teléfono' => !empty($cliente['phone']) ? $cliente['phone'] : 'N/A',
                    'Compras' => $purchases,
                    'Total Gastado' => evaluar(floatval(!empty($cliente['total']) ? $cliente['total'] : 0))
                ];
            }
        } else {
            $totalClientes = 45;
    ...
    function apiDashboard() {
        $sucursal = !empty($_POST['sucursal']) ? $_POST['sucursal'] : $_SESSION['SUB'];
        $mes1 = intval(!empty($_POST['mes1']) ? $_POST['mes1'] : date('n'));
        $anio1 = intval(!empty($_POST['anio1']) ? $_POST['anio1'] : date('Y'));
        $mes2 = intval(!empty($_POST['mes2']) ? $_POST['mes2'] : date('n'));
        $anio2 = intval(!empty($_POST['anio2']) ? $_POST['anio2'] : date('Y') - 1);
        ...
        foreach ($labels as $label) {
            $data2[] = !empty($data2Map[$label]) ? $data2Map[$label] : 0;
        }
        ...
        foreach ($daysOrder as $index => $day) {
            ...
            $weekdayData1[] = !empty($weekdayMap1[$day]) ? $weekdayMap1[$day] : 0;
            $weekdayData2[] = !empty($weekdayMap2[$day]) ? $weekdayMap2[$day] : 0;
        }
        ...
        foreach ($ordersByWeekday1 as $day) {
            $topWeek[] = [
                'dia' => !empty($daysTranslation[$day['dia']]) ? $daysTranslation[$day['dia']] : $day['dia'],
    ...
    function logHistory($message, $type = 'general', $title = 'Registro de actividad') {
        $history = [
            ...
            'type'          => $type,
            'usr_users_id'  => !empty($_SESSION['USR']) ? $_SESSION['USR'] : 1,
        ];
    ...
    function updateDeliveryStatus() {
        ...
        $is_delivered = !empty($_POST['is_delivered']) ? $_POST['is_delivered'] : null;
    ...
    function getDailyClose() {
        ...
        if ($summary && $summary['total_orders'] > 0) {
            ...
            $data    = [
                'quotation_count'      => !empty($summary['quotation_count']) ? $summary['quotation_count'] : 0,
                'cancelled_count'      => !empty($summary['cancelled_count']) ? $summary['cancelled_count'] : 0,
                'pending_count'        => !empty($summary['pending_count']) ? $summary['pending_count'] : 0,
                'subsidiary_name'      => $subsidiary_name,
                'is_all_subsidiaries'  => $is_all_subsidiaries
            ];
        }
        ...
        $data['closed_by']      = !empty($closed_by) ? $closed_by : null;
        $data['closed_at']      = !empty($closed_at) ? $closed_at : null;
    ...
    function openShift() {
        if ($_SESSION['ROLID'] == 1) {
            $subsidiaries_id = !empty($_POST['subsidiaries_id']) ? $_POST['subsidiaries_id'] : $_SESSION['SUB'];
        } else {
            $subsidiaries_id = $_SESSION['SUB'];
        }
        ...
        $shift_name     = !empty($_POST['shift_name']) ? $_POST['shift_name'] : null;
        $opening_amount = floatval(!empty($_POST['opening_amount']) ? $_POST['opening_amount'] : 0);
    ...
    function addDiscount() {
        ...
        $id       = $_POST['id'];
        $discount = floatval(!empty($_POST['discount']) ? $_POST['discount'] : 0);
        $info     = !empty($_POST['info_discount']) ? $_POST['info_discount'] : '';
    ...
    function getDiscount() {
        ...
        $totalPay   = !empty($order['total_pay']) ? $order['total_pay'] : 0;
        $discount   = floatval(!empty($order['discount']) ? $order['discount'] : 0);
        // $totalPaid  = floatval(order total_paid or 0);
        ...
        return [
            ...
            'data'        => [
                ...
                'total_pay'    => $totalPay,
                'discount'     => $discount,
                'info_discount'=> !empty($order['info_discount']) ? $order['info_discount'] : '',
    ...
    function editDiscount() {
        ...
        $id          = $_POST['id'];
        $newDiscount = floatval(!empty($_POST['discount']) ? $_POST['discount'] : 0);
        $info        = !empty($_POST['info_discount']) ? $_POST['info_discount'] : '';
        ...
        $totalPay    = floatval($order['total_pay']);
        $oldDiscount = floatval(!empty($order['discount']) ? $order['discount'] : 0);
...
function dropdownOrder($id, $status, $discount = 0) {
    $instancia = 'app';
    $impresion = 'payment';
    $rolId     = !empty($_SESSION['ROLID']) ? $_SESSION['ROLID'] : 0;
    $owner     = !empty($_SESSION['OWNER']) ? $_SESSION['OWNER'] : 0;
    $hasDiscount = $discount > 0;
...
function renderDeliveryType($deliveryType) {
    $deliveryType = !empty($deliveryType) ? $deliveryType : 'local';
...
function renderDeliveryStatus($order) {
    ...
    $folio       = !empty($order['folio']) ? $order['folio'] : '';
...
function formatSucursal($compania, $sucursal, $numero = null){
    ...
    $number = !empty($numero) ? $numero : rand(1, 99);
