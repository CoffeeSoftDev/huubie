<?php
    ...
    class ctrl extends mdl {
        ...
        function listProductos() {
            ...
            $sesion    = !empty($_SESSION['SUB']) ? $_SESSION['SUB'] : 4;
        ...
        function listClient() {
            ...
            $ls = $this->lsClient([$_POST['active'], $_SESSION['SUB']]);
            ...
            foreach ($ls as $key) {
                ...
                $__row[] = [
                    'id'         => $key['id'],
                    'Nombre'     => renderUserCard($key['name']),
                    'Correo'     => $key['email'],
                    'Teléfono'   => $key['phone'],
        ...
        function listModifier() {
            $estado = !empty($_POST['status']) ? $_POST['status'] : 1;
            ...
            foreach ($modifier as $key) {
                ...
                $includeCalculo = $key['isExtra'] == 1 
                    ? '<span class="text-green-500 text-lg"><i class="icon-ok-circled"></i></span>' 
                    : '';

                $rows[] = [
                    'id'        => $key['id'],
                    'Nombre'    => $key['name'],
                    'Extra'     => $includeCalculo,
...
// Complements.
  ...
  function renderUserCard($name, $color = '#2563EB') {
    ...
    return '<div class="flex items-center gap-3">
        <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm" style="background-color: ' . $color . ';">
            ' . $initials . '
        </div>
        <div class="flex flex-col">
            <span class="text-xs font-semibold ">' . htmlspecialchars($name) . '</span>
        </div>
    </div>';
  }
