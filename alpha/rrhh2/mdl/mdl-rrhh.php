...
class mdl extends CRUD {
    ...
    function lsEmpleados($array) {
        ...
        $query = "
            SELECT e.id, CONCAT(e.nombre, ' ', e.apellido_paterno) AS valor
    ...
    function listPersonal($array) {
        ...
        $query = "
            SELECT e.id, e.codigo_empleado, e.nombre, e.apellido_paterno, e.apellido_materno,
    ...
    function getPersonalById($array) {
        $query = "
            SELECT e.*, p.nombre AS puesto, t.nombre AS turno, s.name AS sucursal
            FROM {$this->bd}rrhh_empleados e
            INNER JOIN {$this->bd}rrhh_puestos p ON p.id = e.puesto_id
            INNER JOIN {$this->bd}rrhh_turnos t ON t.id = e.turno_id
            INNER JOIN {$this->bd}subsidiaries s ON s.id = e.subsidiaries_id
    ...
    function getPersonalCounts($array) {
        ...
        $query = "
            ...
            WHERE {$where}
        ";
        $result = $this->_Read($query, !empty($data) ? $data : null);
        return !empty($result) ? $result[0] : ['total' => 0, 'activos' => 0, 'bajas' => 0, 'suspendidos' => 0];
    }

    function getMaxPersonalCodigo() {
        $query = "
            SELECT MAX(CAST(SUBSTRING(codigo_empleado, 5) AS UNSIGNED)) AS max_num
            FROM {$this->bd}rrhh_empleados
        ";
    ...
    function listPermisos($array) {
        ...
        $query = "
            SELECT p.id, p.codigo, p.tipo, p.fecha_inicio, p.fecha_fin, p.dias, p.razon,
    ...
    function getPermisoById($array) {
        $query = "
            SELECT p.*, CONCAT(e.nombre, ' ', e.apellido_paterno) AS nombre_completo, e.foto_url
            FROM {$this->bd}rrhh_permisos p
            INNER JOIN {$this->bd}rrhh_empleados e ON e.id = p.empleado_id
            WHERE p.id = ?
        ";
    ...
    function listIncidenciaDiario($array) {
        ...
        $query = "
            SELECT i.id, i.fecha, i.tipo_ingreso, i.hora_entrada, i.hora_salida,
    ...
    function listIncidenciaPersonalizado($array) {
        ...
        $query = "
            SELECT i.id, i.empleado_id, i.fecha, i.estatus,
                CONCAT(e.nombre, ' ', e.apellido_paterno) AS nombre_completo,
                e.foto_url,
                p.nombre AS puesto, p.color_badge,
                t.nombre AS turno
    ...
    function getNominaCounts($array) {
        $query = "
            SELECT
                IFNULL(SUM(total_nomina), 0) AS total_general,
                IFNULL(SUM(a_pagar_efectivo), 0) AS total_efectivo,
                IFNULL(SUM(a_pagar_bancos), 0) AS total_bancos,
                COUNT(*) AS total_colaboradores
            FROM {$this->bd}rrhh_nomina_detalle
            WHERE periodo_id = ?
        ";
        $result = $this->_Read($query, [$array['periodo_id']]);
        return !empty($result) ? $result[0] : ['total_general' => 0, 'total_efectivo' => 0, 'total_bancos' => 0, 'total_colaboradores' => 0];
    }
    ...
    function getEmpleadosActivosBySucursal($array) {
        $query = "
            SELECT id, codigo_empleado, nombre, apellido_paterno, salario_diario
            FROM {$this->bd}rrhh_empleados
    ...
    function getMaxNominaPeriodoId() {
        $query = "SELECT MAX(id) AS id FROM {$this->bd}rrhh_nomina_periodos";
        $result = $this->_Read($query);
    ...
    function getResumenCounts($array) {
        ...
        ";
        $result = $this->_Read($query, !empty($data) ? $data : null);
        return !empty($result) ? $result[0] : ['total_empleados' => 0, 'activos' => 0, 'bajas' => 0, 'altas_periodo' => 0, 'bajas_periodo' => 0];
    }
    ...
    function getResumenPermisosPendientes($array) {
        ...
        $query = "
            SELECT p.id, p.codigo, p.tipo, p.fecha_inicio, p.fecha_fin, p.estatus,
                CONCAT(e.nombre, ' ', e.apellido_paterno) AS nombre_completo,
                pu.nombre AS puesto, pu.color_badge
            FROM {$this->bd}rrhh_permisos p
            INNER JOIN {$this->bd}rrhh_empleados e ON e.id = p.empleado_id
