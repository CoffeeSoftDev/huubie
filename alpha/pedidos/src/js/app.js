...
class App extends Templates {
    ...
    // Snapshot fresco de las sucursales de cobro (shift_opened_at). El init lo carga
    // una sola vez y con el se decide el candado de turno del modal de pagos: si otra
    // sucursal abrio su turno despues de cargar la pagina, sin esto seguiria bloqueada.
    async refreshSubsidiariesCobro() {
        const res = await useFetch({ url: this._link, data: { opc: "getSubsidiariesShift" } });
        if (res && Array.isArray(res.data) && res.data.length) subsidiariesCobro = res.data;
    }
   ...
   jsonOrder() {
    ...
    orderFields.push(
        {
            ...
            id: "lblCliente",
            class: "col-12 mb-1",
            html: `<h3 class="text-base font-bold text-white border-b border-gray-600/60 pb-1.5">Información del cliente</h3>`
        },
        {
            opc: "input",
    ...
    applyCobroShiftLock() {
        const sub = (subsidiariesCobro || []).find(s => String(s.id) === String($('#payment_subsidiaries_id').val() || ''));
    ...
    // Repinta el panel de informacion del modal de detalles con datos frescos, sin
    // cerrarlo ni volver a armar el modal completo.
    async refreshOrderDetails(orderId) {
        ...
        $('#orderInfoPanel .lg\\:sticky').html(this.detailsCard(orderData, paymentMethods, payments));
    ...
    // Edicion acotada desde el detalle del pedido: solo los datos de entrega (fecha,
    // hora, tipo y descripcion). El cliente y los productos siguen editandose con
    // editOrder, que abre el POS completo.
        ...
        createCoffeeModalForm({
            ...
            onConfirm: async (data, modal) => {
                ...
                if (response.status == 200) {
                    ...
                    // La fecha y la hora de entrega son columnas del listado.
                    this.ls();
                } else {
                    alert({
                        icon: "error",
    ...
    // mismo flujo que editOrderPaid. Se valida aqui para no descubrir una clave mal
    // escrita hasta despues de capturar los cambios; la autorizacion real la vuelve a
    // exigir editOrderDelivery() del controlador al guardar.
    ...
    // Un pedido liquidado (3) lo corrigen admin (1) y supervisores (6, 7), y ahi la
    // edicion pide contraseña; el resto de los pedidos, cualquiera que opere la lista.
    // El cancelado (4) no se edita.
    editDeliveryIcon(orderData) {
        const liquidado  = orderData.status == '3';
        const autorizado = rol == 1 || rol == 6 || rol == 7;
    ...
    // El formato del ticket de turno vive en shift-ticket.js (fuente unica, compartida
    // con el modulo order-visor). Aqui solo se inyecta el modo de reporte del modulo.
    ticketShiftClose(options) {
        renderShiftTicket({ ...options, reportMode: this.reportMode });
