...
/* ============================================================================
   createCoffeeModalForm — modal propio Huubie (parte del framework CoffeeSoft)
   ----------------------------------------------------------------------------
   ...
   campos en cajas, footer con botones). Vive aqui (antes en el archivo suelto
   pedidos/src/js/coffee-modal-form.js) para estar disponible en todo modulo que
   cargue coffeeSoft.js, sin script adicional.

   Disponible como:
     - this.createCoffeeModalForm({...})   (clases que extienden Components/Templates)
     - createCoffeeModalForm({...})         (funcion global, clases sueltas)

   options = {
     id:          'frmModal',          // prefijo de ids de campos
     title:       'Formulario',
     icon:        'icon-doc-text',     // clase del icono del header (o iconSvg)
     ...
     hideHeader:  false,   // oculta header lateral (icono+titulo+X) -> dialogo centrado
     ...
     json: [
       ...
       { opc:'select', id, lbl, value, data:[{id,valor}], required },  // desplegable
       { opc:'html',    html }                                   // bloque libre, sin caja ni label
     ],
...
(function () {
    ...
    function buildModal(options) {
        ...
        const fieldHtml = (f) => {
            ...
            // Desplegable: data=[{id,valor}], value preselecciona por id. Se lee con
            // collect()/validate() igual que un input (mismo id y data-field).
            if (f.opc === 'select') {
                const placeholderOpt = f.placeholder
                    ? `<option value="" disabled ${f.value == null ? 'selected' : ''}>${f.placeholder}</option>`
                    : '';
                const optionsHtml = (f.data || []).map(it => {
                    const sel = String(it.id) === String(f.value) ? 'selected' : '';
                    return `<option value="${it.id}" ${sel}>${it.valor}</option>`;
                }).join('');
                const chevron = `<span class="absolute inset-y-0 right-0 flex items-center pr-2.5 pointer-events-none ${labelCls}"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></span>`;
                return `<div>${lbl}<div class="relative"><select id="${o.id}-${f.id}" data-field="${f.id}" class="w-full ${inputCls} border rounded-lg pl-3 pr-8 py-2.5 text-sm focus:outline-none appearance-none cursor-pointer" ${f.required ? 'required' : ''}>${placeholderOpt}${optionsHtml}</select>${chevron}</div></div>`;
            }
