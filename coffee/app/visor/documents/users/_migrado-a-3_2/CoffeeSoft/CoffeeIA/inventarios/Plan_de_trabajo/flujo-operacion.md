...
> **Producto:** Huubie · Inventarios (POS-2) **Ámbito de este documento:** explicar, en lenguaje de negocio, qué hace cada pantalla del módulo, cómo se conectan entre ellas, qué actores participan y qué datos viajan. **Estado actual:** todo el módulo opera en **modo FAKE** — los datos vienen de archivos `sample_*.js`, no hay backend conectado. **Empresa de referencia:** Reginas (pastelería con varias sucursales: Kafeto, Central Gpe, Cuarta, Altozano, etc.). **Fecha:** 2026-05-19

---

## 1\. Vista de de pajaro
...
Reginas tiene hoy **97 productos reales** sembrados desde `fayxzvov_reginas.order_products`, repartidos en 8 categorías. Ver [sample\_admin-inventarios.js](../src/js/sample_admin-inventarios.js).
...
-   **97 productos** reales en `SAMPLE_PRODUCTOS_DB` (extraídos de `fayxzvov_reginas.order_products`, subsidiaries\_id=4 = Reginas).
...
-   **7 almacenes** mapeados desde sucursales activas de `fayxzvov_alpha.subsidiaries` (companies\_id=4): Marinni Centro, Club Campestre, Reginas Central Gpe, Altozano, Club Campestre Tap, Reginas Cuarta, Reginas Kafeto.
