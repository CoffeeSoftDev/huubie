...
const PG_STORE_KEY  = pgScoped('studio:settings:v1');
...
async function pgSend(text, images, docs) {
    ...
    // `if (pg.isBusy) return`) y el Studio "deja de responder". El finally garantiza que
    // pgFinish() SIEMPRE corra y devuelva el control al usuario.
    try {
        await stream.drain();
        ...
        pgFinish();   // SIEMPRE libera isBusy — sin esto el Studio quedaría mudo
    }
}
