    ...
    createMenuItems(menuItems) {
        return menuItems
            .map((item) => {
                    ...
                    return `<li class="submenu text-gray-400">
                        <button class="submenucito w-full text-left flex items-center justify-between text-base font-medium px-3 py-2 rounded-lg hover:bg-gray-700 hover:text-white transition">
                            <span class="flex items-center gap-2">
                                <img src="/app/src/img/ventas-menu.svg" alt="" class="w-5 h-5" />
                                ${item.text}
                            </span>
                ...
                return `
                <li class='rounded-lg'>
                    <a href="${item.url}" class="block text-sm text-gray-400 hover:text-white hover:bg-gray-700 transition px-3 py-2 rounded-lg">
                    <span class="flex items-center gap-2">
    ...
    highlightCurrentRoute() {
        ...
        $("#menuSidebar a").each(function () {
            ...
            if (url.attr("href") === currentUrl) {
                url.addClass("text-white font-bold");
                url.closest("ul").slideDown(300);
                url.closest("li").addClass("text-white");
                url.closest("li").addClass("bg-[#374151]");
            }
        });
    }
}
...
$(async () => {
    rutes = await useFetch({ url: "/app/access/ctrl/ctrl-access.php", data: { opc: 'sidebar' } });
