...
let app, salesHistory, saleDetailView;
...
class App extends Templates {
    ...
    render() {
        ...
        salesHistory = new SalesHistory(this._link, this._divModule);
        saleDetailView = new SaleDetailView(this._link, this._divModule);
        salesHistory.dataInit = this.dataInit;
        salesHistory.subId = this.subId;
        saleDetailView.dataInit = this.dataInit;

        this.filterBar();
        this.renderHeader(SAMPLE_VIEW_HEADER);
        this.renderFooter(SAMPLE_VIEW_FOOTER);

        saleDetailView.renderDetail();
        this.populateFilters();
        salesHistory.lsVentas();
        salesHistory.lsKpis();
    }
    ...
    // -- Delegators (compatibilidad con onclick/onchange embebidos en HTML/PHP) --

    onChangeFilters() { return salesHistory.onChangeFilters(); }
    onChangePeriodo() { return salesHistory.onChangePeriodo(); }
    cancelVenta(id)   { return salesHistory.cancelVenta(id); }
    editVenta(id)     { return salesHistory.editVenta(id); }
    getVenta(id)      { return saleDetailView.getVenta(id); }
    renderDetail(sale){ return saleDetailView.renderDetail(sale); }

    // -- Branch change (navbar) --
    ...
    async onBranchChange() {
        ...
        saleDetailView.renderDetail(null);
        salesHistory.lsVentas();
        await salesHistory.lsKpis();
    }
...
class SalesHistory extends App {

    onBranchChange() { /* handled by App orchestrator */ }
    ...
    async cancelVenta(id) {
        const creds = await this.askAdminKey({
            ...
            message:     'Esta venta será cancelada. Una vez realizada esta acción será imposible regresar atrás. Ingresa la clave de un administrador para continuar.',
        ...
        const response = await useFetch({
            url:  this._link,
            data: { opc: 'cancelVenta', id, key: creds.key }
        });
    ...
    reprintTicket(v) {
        ...
        const descuento = (v.descuento != null)
            ...
            : items.reduce((s, it) => s + (it.price || 0) * (it.qty || 0) * ((it.discount || 0) / 100), 0);
...
class SaleDetailView extends App {
    ...
    renderDetail(sale) {
        this.saleDetailPanel({
            ...
            onReabrir: (v) => { if (v) salesHistory.reopenVenta(v.id); },
            onReimprimir: (v) => { if (v) salesHistory.reprintTicket(v); },
            onCancelar: (v) => { if (v) salesHistory.cancelVenta(v.id); }
        });
    }

    // -- Components: saleDetailPanel --
    ...
    saleDetailPanel(options) {
        ...
        const items = v.items || [];
        const isCancelled = String(v.estatus || '').toLowerCase() === 'cancelado';
        const cancelBtnCls = isCancelled
            ? 'flex items-center justify-center gap-1 px-2 py-2 rounded-md border border-[var(--cs-border,#374151)] text-[11px] text-[var(--cs-text-muted,#9CA3AF)] opacity-50 cursor-not-allowed'
            : 'flex items-center justify-center gap-1 px-2 py-2 rounded-md border border-[var(--cs-danger,#E02424)]/40 text-[11px] text-[var(--cs-danger,#E02424)] hover:bg-[var(--cs-danger,#E02424)]/10 transition-colors';
        const cancelBtnAttr = isCancelled ? 'disabled title="Venta ya cancelada"' : '';
        ...
        aside.html(`
            ...
            <div class="px-5 py-3 border-t border-[var(--cs-border,#374151)] flex-shrink-0">
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <button id="${opts.id}_btnCancelar" ${cancelBtnAttr} class="${cancelBtnCls}">
        ...
        $(`#${opts.id}_btnCancelar`).on('click', () => { if (!isCancelled) opts.onCancelar(v); });
    }
}
