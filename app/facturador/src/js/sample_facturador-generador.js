...
const SAMPLE_GENERADOR_HEADER_CLASSES = {
...
const _accionGen = (e) => {
    ...
    if (e.tasa === 0) {
        return [
            {
                class:   (e.generado ? 'btn-primary' : 'btn-ghost') + ' !px-2.5 !py-1 text-[11px]',
...
const _generadorRow = (e) => ({
    id:      e.id,
    Nota:    `<span class="font-bold text-ink-800">#${e.orden}</span>`,
    ID:      `<span class="font-mono text-[10px] text-ink-500">${e.id}</span>`,
    Tasa:    _badgeTasa(e.tasa),
    Estado:  _badgeEstadoGen(e),
    Monto:   `<span class="font-semibold text-ink-900">${_fmtMX(e.total)}</span>`,
    a:       _accionGen(e)
});

const SAMPLE_GENERADOR_TABLE = {
    row: Object.values(SAMPLE_GENERADOR_DB)
...
const SAMPLE_GENERADOR_DIAS = [
    { id: '2026-06-10', valor: '10/06/2026' },
    { id: '2026-06-11', valor: '11/06/2026' },
    { id: '2026-06-12', valor: '12/06/2026' }
];

const SAMPLE_GENERADOR_COUNTS = (() => {
    const all = Object.values(SAMPLE_GENERADOR_DB).filter(e => e.metodo !== 'Efectivo');
    return {
        total:      all.length,
        bloqueados: all.filter(e => e.fiscal === 'invoiced').length,
        cero:       all.filter(e => e.tasa === 0 && e.fiscal !== 'invoiced').length
    };
})();
