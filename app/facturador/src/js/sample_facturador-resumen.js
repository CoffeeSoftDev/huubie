...
const SAMPLE_RESUMEN_HEADER_CLASSES = {
...
const _cellMonto = (n) => `<span class="font-semibold text-ink-900">${_fmtMX(n)}</span>`;

const _checkPendiente = (e, checked) => `<input type="checkbox" class="chk-pending accent-brand-500" data-id="${e.id}" data-amount="${e.total}" onchange="app.onTogglePendiente(this)" ${checked ? 'checked' : ''}>`;
...
const SAMPLE_RESUMEN_TABLE = {
    row: Object.values(SAMPLE_RESUMEN_DB).map(_resumenRowTodos)
};
...
const SAMPLE_RESUMEN_COUNTS = (() => {
    const all      = Object.values(SAMPLE_RESUMEN_DB);
    const total    = all.reduce((s, e) => s + Number(e.total || 0), 0);
    const facturado = all.filter(e => e.fiscal === 'invoiced').reduce((s, e) => s + Number(e.total || 0), 0);
    return {
        tickets:    all.length,
        total:      total,
        facturado:  facturado,
