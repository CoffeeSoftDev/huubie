-- ════════════════════════════════════════════════════════════════════════
--  Huubie · Módulo Inventarios — Fase 1 (POS)
--  Esquema destino: fayxzvov_reginas
--  Excluye Fase 2 (Insumos: supply_*, consumption_area, outflow_reason).
--  Generado desde propuesta-bd.md §3.2 – §3.6 + §3.4.bis + §5.2.
-- ════════════════════════════════════════════════════════════════════════
...
CREATE TABLE `warehouse_area` (
    ...
    CONSTRAINT `warehouse_area_ibfk_1` FOREIGN KEY (`companies_id`)
        REFERENCES `fayxzvov_admin`.`companies`(`id`) ON UPDATE CASCADE
...
CREATE TABLE `warehouse` (
    ...
    KEY `companies_id` (`companies_id`),
    KEY `idx_warehouse_default` (`subsidiaries_id`, `is_default`),
    CONSTRAINT `warehouse_ibfk_1` FOREIGN KEY (`warehouse_area_id`)
        REFERENCES `warehouse_area`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `warehouse_ibfk_2` FOREIGN KEY (`subsidiaries_id`)
        REFERENCES `fayxzvov_alpha`.`subsidiaries`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `product_attribute` (
    ...
    KEY `unit_id` (`unit_id`),
    KEY `companies_id` (`companies_id`),
    CONSTRAINT `product_attribute_ibfk_1` FOREIGN KEY (`warehouse_area_id`)
        REFERENCES `warehouse_area`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `product_attribute_ibfk_2` FOREIGN KEY (`unit_id`)
        REFERENCES `unit`(`id`) ON UPDATE CASCADE,
    CONSTRAINT `product_attribute_ibfk_3` FOREIGN KEY (`product_id`)
        REFERENCES `order_products`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `adjustment_reason` (
    ...
    `icon`          VARCHAR(40) NULL,
    `color_hex`     VARCHAR(7)  NULL,
    `affects_cost`  TINYINT(1)  NOT NULL DEFAULT 0,
    `created_at`    DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `active`        TINYINT(1)  NOT NULL DEFAULT 1,
    PRIMARY KEY (`id`),
...
CREATE TABLE `stock` (
    ...
    KEY `companies_id` (`companies_id`),
    CONSTRAINT `stock_ibfk_1` FOREIGN KEY (`warehouse_id`)
        REFERENCES `warehouse`(`id`) ON UPDATE CASCADE,
    CONSTRAINT `stock_ibfk_2` FOREIGN KEY (`product_id`)
        REFERENCES `order_products`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `inventory_inflow` (
    ...
    `companies_id`       INT          NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_inventory_inflow_folio_company` (`folio`, `companies_id`),
    ...
    CONSTRAINT `inventory_inflow_ibfk_1` FOREIGN KEY (`inflow_origin_id`)
        REFERENCES `inflow_origin`(`id`) ON UPDATE CASCADE,
    ...
    CONSTRAINT `inventory_inflow_ibfk_5` FOREIGN KEY (`user_id`)
        REFERENCES `fayxzvov_alpha`.`usr_users`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `detail_inventory_inflow` (
    ...
    CONSTRAINT `detail_inventory_inflow_ibfk_2` FOREIGN KEY (`inventory_inflow_id`)
        REFERENCES `inventory_inflow`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `inventory_shrinkage` (
    ...
    `created_at`             DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`             DATETIME     NULL ON UPDATE CURRENT_TIMESTAMP,
    `status`                 ENUM('Aplicada','Cancelada') NOT NULL DEFAULT 'Aplicada',
    `active`                 TINYINT(1)   NOT NULL DEFAULT 1,
    ...
    `companies_id`           INT          NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_inventory_shrinkage_folio_company` (`folio`, `companies_id`),
    ...
    CONSTRAINT `inventory_shrinkage_ibfk_1` FOREIGN KEY (`shrinkage_reason_id`)
        REFERENCES `shrinkage_reason`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `detail_inventory_shrinkage` (
    ...
    CONSTRAINT `detail_inventory_shrinkage_ibfk_2` FOREIGN KEY (`inventory_shrinkage_id`)
        REFERENCES `inventory_shrinkage`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- ─────────────────────────────────────────────────────────────────────────
-- §3.4.3 · Traspasos (TRA-####) + historial
-- ─────────────────────────────────────────────────────────────────────────
...
CREATE TABLE `inventory_transfer` (
    ...
    `companies_id`                  INT          NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_inventory_transfer_folio_company` (`folio`, `companies_id`),
    ...
    CONSTRAINT `inventory_transfer_ibfk_1` FOREIGN KEY (`status_id`)
        REFERENCES `transfer_status`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `detail_inventory_transfer` (
    ...
    CONSTRAINT `detail_inventory_transfer_ibfk_2` FOREIGN KEY (`inventory_transfer_id`)
        REFERENCES `inventory_transfer`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `inventory_adjustment` (
    ...
    `created_at`               DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`               DATETIME     NULL ON UPDATE CURRENT_TIMESTAMP,
    `status`                   ENUM('Pendiente','Aplicado','Cancelado') NOT NULL DEFAULT 'Aplicado',
    `active`                   TINYINT(1)   NOT NULL DEFAULT 1,
    ...
    `companies_id`             INT          NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_inventory_adjustment_folio_company` (`folio`, `companies_id`),
    ...
    CONSTRAINT `inventory_adjustment_ibfk_1` FOREIGN KEY (`adjustment_reason_id`)
        REFERENCES `adjustment_reason`(`id`) ON UPDATE CASCADE,
...
CREATE TABLE `detail_inventory_adjustment` (
    ...
    CONSTRAINT `detail_inventory_adjustment_ibfk_2` FOREIGN KEY (`inventory_adjustment_id`)
        REFERENCES `inventory_adjustment`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- ─────────────────────────────────────────────────────────────────────────
-- §3.4.bis · Formatos preguardados de entradas (plantillas)
-- ─────────────────────────────────────────────────────────────────────────
...
CREATE TABLE `inflow_format` (
    ...
    KEY `user_id` (`user_id`),
    KEY `companies_id` (`companies_id`),
    KEY `idx_inflow_format_scope` (`scope`),
    CONSTRAINT `inflow_format_ibfk_1` FOREIGN KEY (`inflow_origin_id`)
        REFERENCES `inflow_origin`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `inflow_format_ibfk_2` FOREIGN KEY (`subsidiaries_id`)
        REFERENCES `fayxzvov_alpha`.`subsidiaries`(`id`) ON UPDATE CASCADE,
    CONSTRAINT `inflow_format_ibfk_3` FOREIGN KEY (`user_id`)
        REFERENCES `fayxzvov_alpha`.`usr_users`(`id`) ON UPDATE CASCADE,
    CONSTRAINT `inflow_format_ibfk_4` FOREIGN KEY (`companies_id`)
        REFERENCES `fayxzvov_admin`.`companies`(`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `detail_inflow_format` (
    ...
    CONSTRAINT `detail_inflow_format_ibfk_2` FOREIGN KEY (`inflow_format_id`)
        REFERENCES `inflow_format`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- ─────────────────────────────────────────────────────────────────────────
-- §5.2 · Tabla auxiliar de folios secuenciales
-- ─────────────────────────────────────────────────────────────────────────
...
-- ════════════════════════════════════════════════════════════════════════
--  FIN — 22 tablas + 1 view + seeds. Fase 2 (Insumos) NO incluida.
-- ════════════════════════════════════════════════════════════════════════
